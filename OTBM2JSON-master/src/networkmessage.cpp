...
{
  if(!item || !(item->getID() >= 100 && item->getID() <= 2570))
     return;
...
void NetworkMessage::AddLight(const Creature *player) { 
  AddByte(0x8D); // 8D - Light Creature Command
  AddU32(player->getID()); // The player with the light
  AddByte(player->lightLevel); // Light Level
  AddByte(983); // Light Color 
}
